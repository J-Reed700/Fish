...
export class HumidAirModifier {
  ...
  apply(position: Vector2D, velocity: Vector2D, deltaTime: number): Vector2D {
    ...
    const noiseScale = this.modifier.noiseScale || 0.008;
    const turbulence = this.modifier.turbulence || 0.15;
    ...
    const noiseY = this.simplexNoise(
      position.x * noiseScale + 50,
      position.y * noiseScale + 50,
      this.time * 0.0008
    );

    const baseDirection = this.modifier.direction || { x: 0.2, y: 0.1 };

    const flowDirection = {
      x: baseDirection.x + noiseX * turbulence,
      y: baseDirection.y + noiseY * turbulence,
    };
