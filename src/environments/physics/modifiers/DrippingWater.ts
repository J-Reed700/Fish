...
export class DrippingWaterModifier {
  ...
  apply(position: Vector2D, velocity: Vector2D, deltaTime: number): Vector2D {
    ...
    const noiseScale = this.modifier.noiseScale || 0.003;
    const turbulence = this.modifier.turbulence || 0.05;
    ...
    const baseDirection = this.modifier.direction || { x: 0, y: 1 };
    ...
    const force = Vector.multiply(flowDirection, this.modifier.strength);
