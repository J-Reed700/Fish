...
export class StrongWindModifier {
  ...
  apply(position: Vector2D, velocity: Vector2D, deltaTime: number): Vector2D {
    ...
    const noiseScale = this.modifier.noiseScale || 0.015;
    const turbulence = this.modifier.turbulence || 0.3;
    ...
    const gustFactor = Math.sin(this.time * 0.002) * 0.5 + 0.5;

    const baseDirection = this.modifier.direction || { x: 1, y: 0.1 };
    ...
    const strengthWithGusts = this.modifier.strength * (1 + gustFactor * 0.6);
    const force = Vector.multiply(normalizedFlow, strengthWithGusts);
