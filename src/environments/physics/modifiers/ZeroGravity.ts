...
export class ZeroGravityModifier {
  ...
  apply(position: Vector2D, velocity: Vector2D, deltaTime: number): Vector2D {
    ...
    const noiseScale = this.modifier.noiseScale || 0.005;
    const turbulence = this.modifier.turbulence || 0.1;
    ...
    const noiseY = this.simplexNoise(
      position.x * noiseScale + 300,
      position.y * noiseScale + 300,
      this.time * 0.0005
    );
