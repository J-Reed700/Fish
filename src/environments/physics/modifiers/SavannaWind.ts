...
export class SavannaWindModifier {
  ...
  apply(position: Vector2D, velocity: Vector2D, deltaTime: number): Vector2D {
    ...
    const noiseScale = this.modifier.noiseScale || 0.012;
    ...
    const noiseY = this.simplexNoise(
      position.x * noiseScale + 150,
      position.y * noiseScale + 150,
      this.time * 0.001
    );

    const sweepFactor = Math.sin(this.time * 0.0015) * 0.3 + 0.7;

    const baseDirection = this.modifier.direction || { x: 0.8, y: 0.05 };
    ...
    const strengthWithSweep = this.modifier.strength * sweepFactor;
    const force = Vector.multiply(normalizedFlow, strengthWithSweep);
